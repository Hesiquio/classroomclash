            ...
            <div id="participant-card-{{ $participant->id }}" class="participant-card {{ $index < 3 ? 'top-rank rank-' . ($index + 1) : '' }} {{ $participant->user_id === Auth::id() ? 'current-user' : '' }}">
                ...
                @if(Auth::user()->isDocente() && $challenge->is_active)
                    <div class="card-actions">
                        ...
                        <button type="button" class="btn-icon validate {{ $participant->finished_at ? 'validated' : '' }}" onclick="openDeliveryModal({{ $participant->id }}, '{{ $participant->user->name }}', {{ $participant->finished_at ? 'true' : 'false' }})" title="{{ $participant->finished_at ? 'Devolver Trabajo' : 'Entregar Trabajo' }}">
